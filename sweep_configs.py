"""Run SAM3 video on one video under several (prompt, score_thr, new_det_thr) configs.

For each config we:
  1. rewrite the two hardcoded thresholds in sam3/model_builder.py
  2. run make_overlay.py with the prompt
  3. save overlay.mp4 + counts.json into its own subdir
  4. collect a summary line

Usage:
  python3 sweep_configs.py --video /path.mp4 --out-root /path/sweep_dir
"""
import argparse
import json
import os
...
CONFIGS = [
    dict(tag="c1_poker_s02_n07", prompt="poker chip",          score=0.2, new_det=0.7),
    dict(tag="c2_poker_s03_n05", prompt="poker chip",          score=0.3, new_det=0.5),
    dict(tag="c3_stack_s03_n07", prompt="poker chip stack",    score=0.3, new_det=0.7),
    dict(tag="c4_stackof_s03",   prompt="stack of poker chips",score=0.3, new_det=0.7),
    dict(tag="c5_pile_s03",      prompt="pile of poker chips", score=0.3, new_det=0.7),
    dict(tag="c6_casino_s03",    prompt="casino chip",         score=0.3, new_det=0.7),
]
...
def main():
    ...
    for cfg in CONFIGS:
        ...
        print(f"  -> ok={ok} elapsed={elapsed:.0f}s", flush=True)

    # Pretty text summary
    lines = ["tag | prompt | score | new_det | unique | avg | max | nonzero | elapsed"]
    for e in summary:
        lines.append(
            "{tag} | {prompt} | {score} | {new_det} | {u} | {avg} | {m} | {nz} | {el}s".format(
                tag=e["tag"], prompt=e["prompt"],
                score=e["score"], new_det=e["new_det"],
                u=e.get("num_unique_objs", "-"),
                avg=e.get("avg_per_frame", "-"),
                m=e.get("max_per_frame", "-"),
                nz=e.get("nonzero_frames", "-"),
                el=e["elapsed_sec"],
            )
        )
